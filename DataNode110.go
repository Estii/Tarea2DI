...
func PropuestaD(msj *nodos.MessageNode) int64{
	...
	if( flag1c==0 && flag2c ==0 ){
		...
		if(flag1==1 && flag2 == 1){
			cantidad_error = cantidad1 + cantidad3
			msjn := nodos.MessageNode{ Cantidad1:0, Cantidad2:cantidad2+cantidad_error,Cantidad3:0,NombreLibro:nombre_libro,ID: msj.ID}
			fmt.Println("Propuesta Rechazada")
			return PropuestaD(&msjn)
		}
	}
